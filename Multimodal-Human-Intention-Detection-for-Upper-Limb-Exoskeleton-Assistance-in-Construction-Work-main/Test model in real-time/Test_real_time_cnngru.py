...
TXT_FILE = input("Enter the name of the TXT file (without extension) in CNN_GRU folder: ").strip()
TXT_PATH = os.path.join(BASE_DIR, "CNN_GRU", TXT_FILE + ".txt")
...
for i, (sample_name, action, tool, pred_time) in enumerate(parsed_data, start=start_row):
    ...
    if sample_name == "Sample_1":
        test_counter += 1
        ws[f"P{i}"] = test_counter  # Write test number
    else:
        ws[f"P{i}"] = None  # Leave empty if not Sample_1

    # Write number of sample in column Q
    ws[f"Q{i}"] = i - start_row + 1  # Incremental number of sample

    # Write the rest of the data
    ws[f"R{i}"] = action           # Action name
    ws[f"S{i}"] = tool             # Tool used
    ws[f"T{i}"] = float(pred_time) # Prediction time

# --- Compute accuracy ---
expected_action = ws["A2"].value
predicted_actions = [ws[f"R{row}"].value for row in range(start_row, start_row + len(parsed_data))]
...
# Write overall mean of all block averages to V3
ws["V3"] = round(mean(mean_diffs), 3) if mean_diffs else "N/A"

# --- Compute mean of all Sample_1 prediction times ---
first_samples = [times[0] for times in sample_blocks.values() if times]
ws["V5"] = round(mean(first_samples), 3) if first_samples else "N/A"

# --- Write total number of samples ---
ws["V8"] = len(parsed_data)
