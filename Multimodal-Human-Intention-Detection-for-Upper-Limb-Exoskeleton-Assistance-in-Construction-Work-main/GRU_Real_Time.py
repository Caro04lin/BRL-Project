# Real-time prediction code using GRU for painting tasks and for the website
...
try:
    ...
    from Imports.Models.fusion_gru import FusionModelGRU
...
log_file = None
if record_choice == "Y":
    txt_name = input("Enter txt filename (without extension) in GRU folder: ").strip()
    base_dir = os.path.join(os.path.dirname(os.path.abspath(__file__)), "Test model in real-time", "GRU")
...
ModelToLoad_Path = r"D:\Bristol Robotics Laboratory\AI Project\Multimodal-Human-Intention-Detection-for-Upper-Limb-Exoskeleton-Assistance-in-Construction-Work-main\Pre Trained Model\fusion_movinet_gru_final.pt" 
...
model.load_state_dict(torch.load(ModelToLoad_Path, map_location=device))
model.to(device)
model.eval()
...
try: #Main Loop
    ...
    while True:
        ...
        sample_folder = sample_folders[-2]

        # Avoid re-predicting the same sample endlessly
        if sample_folder == last_predicted_sample:
            continue
        ...
        try:
            ...
            with torch.no_grad():
                ...
                label = idx_to_action[prediction.item()]

                final_tool, needs_support = detect_tools_with_fusion_check(
                    yolo_model, frames_tensor, label
                )

            # Display only if different from last displayed label
            if label != last_displayed_label:
                print(
                    f'{sample_folder} : {label} | Tool: {final_tool} '
                    f'at {round(time.time() - Start_Tracking_Time, 2)}'
                )
                last_displayed_label = label  # Update last displayed

            last_predicted_sample = sample_folder  # Update last predicted sample
