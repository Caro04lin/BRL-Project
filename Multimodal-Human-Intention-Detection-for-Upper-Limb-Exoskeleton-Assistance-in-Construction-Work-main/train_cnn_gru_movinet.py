...
def plot_confusion_matrix(cm, class_names):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(SAVE_DIR, "Confusion_Matrix_CNNGRU_2.pdf"))
    plt.show()
...
def plot_precision_recall_curve(all_labels, all_scores, class_names):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(SAVE_DIR, "Precision_Recall_Curve_CNNGRU_2.pdf"))
    plt.show()

# ===============================
# Training
# ===============================
def train_model(model, train_loader, valid_loader, test_loader, criterion, optimizer, scheduler, device, num_epochs):
    model.to(device)
    best_val_acc = 0.0
    patience_counter = 0
    PATIENCE = 3
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            patience_counter = 0
            torch.save(model.state_dict(), os.path.join(SAVE_DIR, "fusion_movinet_cnn_gru_best_2.pt"))
    ...
    print(f"\nFinal Test | Loss: {test_loss:.4f} | Accuracy: {test_acc:.2f}%")
    ...
    results_path = os.path.join(SAVE_DIR, "test_results_cnn_gru_2.pkl")
    with open(results_path, "wb") as f:
        pickle.dump(test_results, f)
    print(f"✅ Test results saved to {results_path}")

    # Save final model
    torch.save(model.state_dict(), os.path.join(SAVE_DIR, "fusion_movinet_cnn_gru_final_2.pt"))
    print(f"✅ Model saved as {os.path.join(SAVE_DIR, 'fusion_movinet_cnn_gru_final_2.pt')}")
...
if __name__ == "__main__":
    ...
    dataset = HARDataSetTrain(
        root_dir="/home/epfstudent/Desktop/Caroline/Bristol Robotics Laboratory/AI Project/Dataset/Action Dataset",
        transform=transform,
        sequence_length=10
    )
    ...
    train_model(model, train_loader, valid_loader, test_loader, criterion, optimizer, scheduler, device, num_epochs=25)
