...
def evaluate_model(model, loader, criterion, device):
    ...
    with torch.no_grad():
        for video_frames, imu_data, labels in loader:
            ...
            all_scores.extend(torch.softmax(outputs, dim=1).detach().cpu().numpy())
...
def plot_confusion_matrix(cm, class_names):
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(SAVE_DIR, 'Confusion_Matrix_gru.pdf'))
    plt.show()
...
def plot_precision_recall_curve(all_labels, all_scores, class_names):
    ...
    plt.grid(True)
    plt.tight_layout()
    plt.savefig(os.path.join(SAVE_DIR, 'Precision_Recall_Curve_gru.pdf'))
    plt.show()

def train_model(model, train_loader, valid_loader, test_loader, criterion, optimizer, scheduler, device, num_epochs, dataset):
    model.to(device)
    best_val_acc = 0.0
    patience_counter = 0
    PATIENCE = 4
    ...
    for epoch in range(num_epochs):
        ...
        if val_acc > best_val_acc:
            ...
            patience_counter = 0
            torch.save(model.state_dict(), os.path.join(SAVE_DIR, "fusion_movinet_gru_best.pt"))
    ...
    results_path = os.path.join(SAVE_DIR, "test_results_gru.pkl")
    with open(results_path, "wb") as f:
        pickle.dump(test_results, f)
    print(f"✅ Test results saved to {results_path}")

    torch.save(model.state_dict(), os.path.join(SAVE_DIR, 'fusion_movinet_gru_final.pt'))
    print(f"Model saved as {os.path.join(SAVE_DIR, 'fusion_movinet_gru_final.pt')}")
...
if __name__ == "__main__":
    ...
    train_loader, valid_loader, test_loader = create_data_loaders(dataset, batch_size=16)
    ...
    train_model(model, train_loader, valid_loader, test_loader, criterion, optimizer, scheduler, device, num_epochs=25, dataset=dataset)
